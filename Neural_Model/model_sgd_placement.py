...
# Chnage the features names
from utils.database import add_placement_features_to_database, fetch_user_id, add_training_mode_placement
...
cookies = CookieManager(key="mode")
#-----------------------------------------------------------------------#
...
def custom_model_training(features, target, dataframe, st, cookie_manager):
    ...
    os.makedirs(f"Main_Models\\{id}\\placement_model", exist_ok=True)
    add_training_mode_placement("custom", cookie_manager.get(cookie='email'))
    model_1.save(f"Main_Models\\{id}\\placement_model\\model")
    #------------Show lottie success----------------------#
...
def default_model_training(dataframe, target, data, st, cookie_manager):
    ...
    if len(features) == 0:
        add_placement_features_to_database(
    ...
    add_training_mode_placement('default', cookie_manager.get(cookie='email'))
    colors = ['blue', 'orange', 'red', 'green']
    data = [go.Bar(x=['Accuracy', 'Precision', 'Recall', 'F1-Score'],
                   y=[accuracy, precision, recall, f1score],
                   marker=dict(
    ...
    os.makedirs(f"Main_Models\\{id}\\placement_model", exist_ok=True)
    joblib.dump(
        model_1, f"Main_Models\\{id}\\placement_model\\model.joblib")
    #------------Show lottie success----------------------#
    st.success("Model is trained and saved to dataset")
    st_lottie(load_lottiefile("assests\\lottianimations\\model-trained.json"),
              key="model success", height=150, width="100%", speed=0.9)
