import time
from Models.train_placement import train_placement
from Models.use_placement import use_placement
...
cookie_manager = stx.CookieManager(key="placementpage")  # Cookie manager
...
if user is None:
    ...
    st.header("Placement Page")
    tab1, tab2 = st.tabs(["Train Model", "Use model"])
    with tab1:
        train_placement(user, cookie_manager)
    with tab2:
        use_placement(user)
