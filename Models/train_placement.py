...
from Neural_Model.model_sgd_placement import default_model_training, custom_model_training
import extra_streamlit_components as stx

# ------------------------Object initializations---------------------------------#
plutil = placement_utils.placement()
# -------------------------------------------------------------------------------#


def train_placement(user, cookie_manager):
    id = db.fetch_user_id(user)
    if path.exists(f"Main_Models\\{id}\\placement_model\\model") or path.exists(f"Main_Models\\{id}\\placement_model\\model.joblib"):
        st.warning(
    ...
    uploaded_file = plutil.file_uploader()
    if uploaded_file is not None:
        data, dataframe = plutil.create_list_frame(uploaded_file)
        with st.expander("Select what you want to predict"):
            inputData = []
            for i in dataframe:
                if len(pd.value_counts(dataframe[i])) == 2:
                    inputData.append(i)
            target = dataframe[plutil.radiobox_contanier(inputData)]
        ...
        with st.expander("Select features on basis of which you want to buld model"):
            ...
            if c1 == 'Custom features':
                try:
                    ...
                    plutil.checkbox_container(data, target)
                    c = plutil.get_selected_checkboxes()
                    features = dataframe[c]
                    db.add_placement_features_to_database(user, c)
