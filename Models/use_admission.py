...
from utils.database import fetch_admission_features_from_database, fetch_user_id,retrive_training_mode_admission
...
def get_graph(graph_df,i,colors):
    bar = go.Figure(go.Bar(x=graph_df[i], y=graph_df['Admitted'], marker=dict(color=colors)))
    bar.update_yaxes(title="percentage")
    bar.update_layout(
        yaxis=dict(
        title="Percentage",
        range=[0, graph_df['Admitted'].max()]
        ),
        xaxis=dict(
        title=f"{i}"
        )
    )
    bar.update_traces(text=graph_df['Admitted'].apply(
        lambda x: round((x/graph_df["Admitted"].sum())*100, 2)), textposition='auto')
    #-----------Draw doughnt chart-------------------------------#
    pie = go.Figure(go.Pie(labels=graph_df[i], values=graph_df['Admitted'], hole=.5, marker=dict(
...
def use_admission(user):
    st.set_option('deprecation.showPyplotGlobalUse', False)
    id = fetch_user_id(user)
    if not path.exists(f"Main_Models\\{id}\\admission_model"):
        st.error("No model has been trained")
        return 
    st.header("Choose CSV file")
    uploaded_file = st.file_uploader("", key="use_admisison")
    if uploaded_file is not None: 
        #----------------To acess the fetched columns into list we have used this eval function--------------#
        features = fetch_admission_features_from_database(user)
        ...
        if uploaded_file is not None:
            ...
            try:                    
                ...
                if retrive_training_mode_admission(user) == "custom": 
                    loaded_model = load_model(f"Main_Models\\{id}\\admission_model\\model")
                elif retrive_training_mode_admission(user) == "default":
                    loaded_model=joblib.load(f"Main_Models\\{id}\\admission_model\\model.joblib")
                else:
                    st.error("No model has been trained")
                    return
                ...
                for i in sections:
                    with st.expander(f"{i}"):
                        ...
                        final_df["Admitted"] = final_df['Admitted'].apply(
                            lambda x: round((x/final_df["Admitted"].sum())*100, 2))
                    
                        #---------------------------------If we have more than 5 columns then only show top 5 columns------------------------------------#
                        if len(final_df) > 5:
                            counts = final_df.sort_values(by="Admitted",ascending=False)
                            graph_df=counts.drop_duplicates(subset=['Admitted'])[:5]
                            ...
                            #-----------------Draw bar graph----------------------------#
                            bar = go.Figure(
                                go.Bar(x=graph_df[i], y=graph_df['Admitted'], marker=dict(color=colors)))
                            bar.update_yaxes(title="percentage")
                            bar.update_layout(
                                yaxis=dict(
                                title="Percentage",
                        ...
                        #--------------------------------------If we less than five columns---------------------------#
                            ...
                            #-----------------Draw bar graph----------------------------#
                            bar = go.Figure(
                                go.Bar(x=final_df[i], y=final_df['Admitted'], marker=dict(color=colors)))
                            bar.update_yaxes(title="percentage")
                            bar.update_layout(
                                yaxis=dict(
                                title="Percentage",
                                range=[0, final_df['Admitted'].max()]
                                ),
                                xaxis=dict(
                                    title=f"{i}"
                                )
                            )
                            bar.update_traces(text=final_df['Admitted'].apply(
                                lambda x: round((x/final_df["Admitted"].sum())*100, 2)), textposition='auto')
                            #-----------Draw doughnt chart-------------------------------#
                            pie = go.Figure(go.Pie(labels=final_df[i], values=final_df['Admitted'], hole=.5, marker=dict(
                                colors=colors), textposition='inside'))
                            pie.update_layout(width=800, height=500)
                            # display the chart
                            st.plotly_chart(bar,use_container_width=True)
